...
pub const RELATIVE_PROTO_DIR_PATH: &str = "src/sonic_config/schemas/proto";

/// The relative proto output directory path from the cargo.toml
pub const RELATIVE_PROTO_OUT_DIR_PATH: &str = "src/sonic_config/proto_codegen";
...
fn collect_proto_files(dir: &Path) -> Result<Vec<PathBuf>, Box<dyn std::error::Error>> {
    ...
    let entries = match read_dir(dir) {
        Ok(entries) => entries,
        Err(error) => {
            error!("[evm-etl] Current Dir: {:?}", std::env::current_dir());
            error!(
                "[evm-etl] Failed to read from directory `{:?}` due to {}",
                dir, error
            );
...
fn collect_output_rs_files(
    ...
    for entry in entries {
        let entry = match entry {
            Ok(entry) => entry,
            Err(error) => {
                error!("[evm-etl] Folder: {:?}", std::env::current_dir());
                error!("[evm-etl] Failed to get file: {:?}", error);
                return Err(Box::new(error));
            }
        };
        let path = entry.path();
...
fn build_protos() -> Result<(), ()> {
    ...
    info!("[evm-etl] Starting evm-etl build...");

    let mut config = prost_build::Config::new();

    config.message_attribute(".etl", "#[derive(serde::Serialize, serde::Deserialize)]");
    config.enum_attribute(".etl", "#[derive(serde::Serialize, serde::Deserialize)]");

    info!("[evm-etl] Searching for proto files...");
    let dir = Path::new(RELATIVE_PROTO_DIR_PATH);
    info!("[evm-etl] Searching starting in proto root dir: {:?}", dir);
    let proto_files = match collect_proto_files(dir) {
        Ok(proto_files) => {
            info!("[evm-etl] Collected {} proto files", proto_files.len());
            proto_files
        }
        Err(error) => {
            error!(
                "[evm-etl] Failed to build protos for evm-etl due to: {}",
                error
            );
            panic!(
                "[evm-etl] Failed to build protos for evm-etl due to: {}",
    ...
    match create_dir_all(outdir) {
        Ok(_) => info!("[evm-etl] Successfully located/created output directory."),
        Err(error) => {
            error!(
                "[evm-etl] Error when creating codegen output dir: {}",
                error
            );
            panic!(
                "[evm-etl] Error when creating codegen output dir: {}",
    ...
    match protos_results {
        Ok(_) => info!("Protos for evm-etl created"),
        Err(error) => {
            error!("[evm-etl] Failed to build protos for evm-etl: {}", error);
            panic!("[evm-etl] Failed to build protos for evm-etl: {}", error);
        }
    }

    info!("[evm-etl] Collecting all outputted-rust code");
